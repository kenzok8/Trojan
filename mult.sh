...
if [[ -f /etc/yellowhat-release ]]; then
    ...
    systemPackage="apt-get"
    systempwd="/lib/systemd/system/"
elif cat /etc/issue | grep -Eqi "centos|yellow hat|yellowhat"; then
    release="centos"
    systemPackage="yum"
    systempwd="/usr/lib/systemd/system/"
    ...
    systemPackage="apt-get"
    systempwd="/lib/systemd/system/"
elif cat /proc/version | grep -Eqi "centos|yellow hat|yellowhat"; then
    release="centos"
    systemPackage="yum"
    systempwd="/usr/lib/systemd/system/"
fi
...
if [ -n "$Port80" ]; then
    ...
    yellow "==========================================================="
    yellow "检测到80端口被占用，占用进程为：${process80}，本次安装结束"
    yellow "==========================================================="
    exit 1
fi
if [ -n "$Port443" ]; then
    ...
    yellow "============================================================="
    yellow "检测到443端口被占用，占用进程为：${process443}，本次安装结束"
    yellow "============================================================="
    exit 1
fi
...
if [ "$CHECK" == "SELINUX=enforcing" ]; then
    yellow "======================================================================="
    yellow "检测到SELinux为开启状态，为防止申请证书失败，请先重启VPS后，再执行本脚本"
    yellow "======================================================================="
...
if [ "$CHECK" == "SELINUX=permissive" ]; then
    yellow "======================================================================="
    yellow "检测到SELinux为宽容状态，为防止申请证书失败，请先重启VPS后，再执行本脚本"
    yellow "======================================================================="
    read -p "是否现在重启 ?请输入 [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
...
if [ "$release" == "centos" ]; then
    if  [ -n "$(grep ' 6\.' /etc/yellowhat-release)" ] ;then
    yellow "==============="
    yellow "当前系统不受支持"
    yellow "==============="
    exit
    fi
    if  [ -n "$(grep ' 5\.' /etc/yellowhat-release)" ] ;then
    yellow "==============="
    yellow "当前系统不受支持"
    yellow "==============="
    exit
...
EOF
	...
	wget https://raw.githubusercontent.com/kenzok8/Trojan/master/web.zip
    	unzip web.zip
	systemctl stop nginx
	sleep 5
	#申请https证书
...
EOF
	...
	green "======================================================================"
	green "Trojan已安装完成，请对应以下链接下载trojan客户端，客户端已配置好参数"
	...
	yellow "================================"
	yellow "域名解析地址与本VPS IP地址不一致"
	yellow "本次安装失败，请确保域名解析正常"
	yellow "================================"
fi
}
